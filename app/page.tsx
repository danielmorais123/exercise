...
export default async function Home() {
  ...
  const user: User | undefined = users.find(
    (x) => x.email === cookieEmail.value
  );

  const postsFromUser: Post[] = posts.filter((x) => x.userId === user?.id);
  ...
  return (
    <div className="min-h-screen p-2  gap-2 flex flex-col justify-center items-center">
      <div className="flex items-center gap-2">
        ...
        >
          {" "}
          <button type="submit" className="w-fit bg-yellow-300 text-black py-2 px-4 rounded-md text-sm">
            Sign Out
          </button>
        </form>
